...
export async function syncEmailsToDatabase(emails:EmailMessage[], accountId: string) {
  //100+ more concurrent db connections, need rate limitted function
  //concurrent request batches are needed : pLimit
...
//take response from aurinko and insert it into db
async function upsertEmail(email:EmailMessage, accountId: string, index: number) {
  ...
  try{
    ...
    //1. Upsert EmilAddress records
    // filtering unique email address
    const addressesToUpsert = new Map() // string : object
    for (const address of [email.from, ...email.to, ...email.cc, ...email.bcc, ...email.replyTo]) {
      addressesToUpsert.set(address.address, address)
    }
    
    const upsertedAddresses:(Awaited<ReturnType<typeof upsertEmailAddress>>)[] = [] //to save the db transactions
...
async function upsertEmailAddress(address:EmailAddress, accountId: string) {
  try {
    ...
    console.log("Fail to upsert email address", error)
    return null
  }
}
